...
export interface SdkSpec {
  ...
  /**
   * Package specifier emitted for core imports in generated module headers
   * (`schema`, `api`, `category`). Defaults to `@distilled.cloud/core`;
   * forks that republish core under their own scope set it here.
   */
...
export const generateService = (
  ...
  const defaultHeader = (ctx: { hasPaginated: boolean }): string => {
    ...
    return (
      ...
      `import * as S from "${spec.corePackage ?? "@distilled.cloud/core"}/schema";\n` +
      `import * as API from "${spec.corePackage ?? "@distilled.cloud/core"}/api";\n` +
      (usesCategories
        ? `import * as C from "${spec.corePackage ?? "@distilled.cloud/core"}/category";\n`
        : "") +
