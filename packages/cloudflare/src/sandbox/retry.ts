/**
 * Cloudflare Sandbox retry surface — a veneer over
 * `@rikalabs/distilled-core/retry`.
 *
 * The `Retry` service tag is threaded into every generated operation via
 * `API.make({ retry: Retry.Retry })`, so a caller-installed policy applies
 * to all Cloudflare Sandbox calls below it and core's `makeDefault` is the
 * fallback when none is provided.
 *
 * @example
 * ```ts
 * import * as CloudflareSandbox from "@rikalabs/distilled-cloudflare";
 *
 * myEffect.pipe(CloudflareSandbox.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of Cloudflare Sandbox API calls. */
export class Retry extends Context.Service<Retry, Policy>()(
  "CloudflareSandboxRetry",
) {}

/** Provides a custom retry policy to every Cloudflare Sandbox API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
