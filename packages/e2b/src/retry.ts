/**
 * E2B retry surface — a veneer over `@rikalabs/distilled-core/retry`.
 *
 ...
 * E2B calls below it and core's `makeDefault` is the fallback when none is
 * provided.
 *
 * @example
 * ```ts
 * import * as E2B from "@rikalabs/distilled-e2b";
 *
 * myEffect.pipe(E2B.Retry.transient);
 * ```
 */
...
/** Context tag for configuring retry behavior of E2B API calls. */
export class Retry extends Context.Service<Retry, Policy>()("E2BRetry") {}

/** Provides a custom retry policy to every E2B API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
...
/** Retry options that retry all transient errors indefinitely (honoring server hints). */
