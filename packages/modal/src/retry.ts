 ...
 * Modal calls below it and core's `makeDefault` is the fallback when none is
 * provided.
 *
 * @example
 * ```ts
...
/** Context tag for configuring retry behavior of Modal API calls. */
export class Retry extends Context.Service<Retry, Policy>()("ModalRetry") {}

/** Provides a custom retry policy to every Modal API call below it. */
export const policy: {
  (
    options: Options,
  ): <A, E, R>(
